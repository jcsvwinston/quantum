...
| NU-50 | P2 | `pkg/model/meta.go:427-509` frente a `quark/internal/schema/schema.go:337-378` | `pkg/model` y quark leen ambos un tag `db` con gramáticas incompatibles, y ninguno avisa. Un modelo estilo nucleus deja a quark creyendo que la tabla tiene una columna llamada literalmente `column:email;unique;not null`, y sin clave primaria. Al revés se pierden `size=` y `pk:"true"` con sólo un WARN de arranque. Un mismo struct no puede servir hoy a las dos capas. | El linter de tags de A4; medido en `docs/pkg-model-vs-quark.md`. |
