...
| OR-45 | P2 | `internal/admin/datastudio.go` (list) ; `internal/adminbench` DS-04 | Toda lista responde `total: -1` con `is_estimated: true` — filtrada o no, y con cinco filas en SQLite—, así que ningún paginador puede decir cuántas páginas hay y la SPA cae a «load more». Medido el 2026-09-12 sobre v1.9.6 por el banco de admin | Contar de verdad cuando el motor puede (y cuando hay filtro, contar el filtro); reservar la estimación para tablas grandes declaradas |
| OR-46 | P2 | `internal/admin/sessions.go:58-70` (`sessionRow.User`) ; `internal/adminbench` OPS-16 | La fila de sesión lleva campo `user` y el panel nunca lo rellena: el operador que revoca una sesión desde el visor no puede saber de quién es. El framework SÍ sabe el sujeto de cada sesión | Resolver el sujeto al construir la fila; es el mismo dato que ya usa la actividad de sesión |
| OR-47 | P3 | `internal/admin/management_migrations.go` ; `internal/adminbench` OPS-17 | Una aplicación sin directorio de migraciones recibe 500 en la vista, no una lista vacía: el error del migrador sube tal cual | Distinguir «no hay dónde mirar» de «falló mirar»: lista vacía con el motivo |
| OR-48 | P3 | `internal/admin/panel.go:444,460` (fallback SPA) ; `internal/adminbench` (cuatro sondas de ausencia lo confundieron con una ruta) | Cualquier ruta no servida bajo el prefijo —`/api/*` incluido— responde 200 `text/html` (la SPA) en GET y 405 en POST: un cliente de API no puede distinguir «no existe ese endpoint» de «aquí hay una página» | Que el fallback no cubra `/api/*`: 404 JSON bajo el prefijo de API |
...
| OR-52 | P3 | `internal/admin/panel.go` (ruta de estado de export) ; `internal/adminbench` OPS-15 | El id de un export es su clave de almacenamiento, que contiene una barra (`_tmp/export_<sello>.csv`), de modo que `GET /api/exports/{id}` —un solo segmento— nunca casó con los ids que el propio panel emite: consultar un export por id caía al fallback de la SPA y volvía como `200 text/html`. El banco lo leyó como un control que funcionaba y registró OPS-15 `present` desde `S0`. Sale a la luz al cerrar OR-48, que es lo que convierte ese 200 en un 404. | **Cerrado en la misma sesión**: la ruta toma `{id...}`; `/api/exports/{id}/download` sigue siendo más específica y gana. Y la lección para el banco, escrita en su página: en este panel un 200 no es evidencia hasta que algo del cuerpo lo es. |
| OR-51 | P3 | `quarkdatasource/store.go` | `quarkdatasource` no implementa `datasource.OperatorFilterSource`, así que una aplicación que monta el panel sobre Quark no puede usar los filtros con operador. No es descuido: su `go.mod` pina la RAÍZ de orbit y el CI construye cada módulo con `GOWORK=off`, de modo que no puede nombrar `datasource.Filter` hasta que la raíz que lo publica tenga tag — la misma dependencia topológica que `check_internal_pins.sh` ya tolera en ese borde. Mientras tanto el panel **rechaza** esas consultas en vez de contestarlas sin filtrar, que es para lo que existe la interfaz. | **HECHO** (orbit#490, tras la release v1.10.0 de la raíz): los doce operadores traducidos al builder de Quark, `in` vacío que no casa nada, operador no expresable rechazado por su nombre en vez de degradar a igualdad, y el rechazo de QK-25 donde el comodín no se puede escapar (SQLite y Oracle). Los tests comprueban QUÉ FILAS vuelven. |
| OR-53 | P2 | `orbit.go:495` (el `PanelConfig` que construye el módulo) ; `internal/admin/panel.go:73` (`TaskInspector`) ; `internal/admin/flags.go:313` | **La vista de colas del panel es inalcanzable en una aplicación real.** El panel sabe pintarla —A6 la entregó— pero `orbit.Module` nunca asigna `TaskInspector`, y `orbit.Config` no tiene campo por donde recibirlo: en cualquier aplicación montada con el módulo, `/admin/api/system` devuelve `jobs.enabled=false` con razón «task inspector not configured (check redis_url)» y toda acción de cola muere en un 400 antes de tocar el proveedor. El texto de la razón conserva además la asunción que lo explica: se dio por hecho que inspeccionar la cola era cosa de Redis | Un campo en `orbit.Config` que acepte un `tasks.Inspector` (aditivo), alimentado por la mitad de nucleus, NU-83: hoy la aplicación tampoco tiene de dónde sacarlo, porque `nucleus.Runtime` expone el `Manager` y no el inspector del proveedor que el propio framework arrancó | **HECHO en el tren de A7**: el inspector se resuelve POR LLAMADA, no se captura. Asignarlo en `OnStart` tampoco habría servido — los `OnStart` de los módulos corren en `nucleus.go:803` y el runtime de jobs se construye en la 884, así que ahí todavía no hay ninguno y un valor leído entonces es nil para toda la vida del proceso. Llega por la interfaz OPCIONAL `nucleus.TaskInspectorFrom`, no por un método en `Runtime`, que está publicado. |
| OR-54 | P2 | `internal/admin/management.go` (`handleJobs`) | El endpoint respondía `enabled: p.config.RedisURL != ""`, es decir **«hay cola» significaba «hay una URL de Redis»**. Eso era cierto sólo mientras toda cola durable necesitaba Redis; A7 publica una sobre la base de datos de la propia aplicación, que no tiene URL de Redis y funciona, y este panel le decía a su operador que los jobs estaban apagados | `enabled` es lo que el inspector contesta, con la URL de Redis al lado para quien esté leyendo un despliegue de Redis |
| OR-55 | P2 | `internal/adminbench/probes_operations_test.go` (`probeJobQueues`, OPS-14) ; `internal/adminbench/env_test.go` (`contentModule`) | **El control que debía cazar OR-53 medía que el endpoint devolviera 200.** Leía `payload["queues"]`, una clave de nivel superior que la respuesta nunca ha tenido —es `{enabled, redis_url, snapshot}`—, así que su única rama de detección era inalcanzable y reportó `present` durante todo A6 sobre un panel ciego. Y peor: la aplicación del banco **no declaraba ningún job**, de modo que no había cola de la que la vista fuera vista. Es AUD-05 otra vez, en el banco de A6 | Que la aplicación del banco declare un job (cada hora: nunca dispara durante una corrida) y que la sonda lea el snapshot de verdad. Quitando el cableado, el banco se pone rojo |
| OR-56 | P2 | `server/services/agent_service.go` (bucle lector de `Stream`) ; `server/nodes/registry.go` (`Add`) | **Un reconexión con el mismo `node_id` no supersede el stream anterior.** `Registry.Add` sustituye la entrada y cancela el escritor del stream viejo, pero el lector del handler bloquea en `stream.Receive()` y sólo mira el contexto cancelado después de un error, así que el stream viejo sigue vivo hasta que su par lo cierre: un evento emitido en el bus del agente superseded DESPUÉS de que el nuevo se registre llega al suscriptor de la UI como si fuera del nodo. `registry_test.go:115` fija la mitad del registro, no la del handler. Lo encontró la ronda adversarial de la `S0` de A9: la sonda `HA-05` contaba entradas de un `map` y no podía verlo | Que el lector salga en `streamCtx.Done()` (Receive en goroutine, select sobre el ctx) para que el handler retorne y el par viejo reciba error; `HA-05` mide el stream, no el mapa, y queda `partial` hasta entonces |
| OR-57 | P3 | `server/config.go` (`AgentIdentityFromCertificate`) ; `server/services/agent_service.go` (`Stream`) | **El enlace de la identidad del nodo al certificado es opt-in.** A9 `S1` (orbit#501) lo añadió: con el knob, un `node_id` distinto del CN del certificado verificado se rehúsa con `PermissionDenied`. Apagado por defecto, el servidor sigue registrando lo declarado —un certificado para `node-a` puede sentarse como `node-b`— y deja un WARN con las dos identidades y el flag. Encenderlo por defecto es rompiente para un fleet que comparte un certificado entre sus agentes | Voltear el default en el major de A12 (QADR-0010), con la ventana de deprecación escrita en la doc de seguridad; hasta entonces `IDENT-06` mide ambos regímenes y la doc dice cuál es cuál |
