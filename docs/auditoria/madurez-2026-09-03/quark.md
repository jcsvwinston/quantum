...
| QK-21 | P1 | `internal/migrate/migrate.go:409-424` | El mapeo de tipos colapsa todo entero Go (`int8`..`int64`, `uint8`..`uint64`) en un solo `INTEGER`, y todo flotante en `REAL` para PostgreSQL y SQLite. Un `int64` no cabe en un `int4` y un `float64` pierde precisión en un `float4`; la PK autoincremental sale `SERIAL`, no `BIGSERIAL`, así que se agota a los 2.147.483.647. `pkg/model` de nucleus emite `BIGINT`/`DOUBLE PRECISION`/`BIGSERIAL` para el MISMO modelo. **El comportamiento por motor no está confirmado contra motores reales**: la medición de A4/S0 no tuvo runtime de contenedores. | **HECHO** (quark#393). Confirmado contra los seis: MySQL rechaza con «Out of range value», PostgreSQL y SQL Server también; Oracle nunca lo tuvo. Los enteros mapean por anchura y las claves son de 64 bits. Nota de migración: `PlanMigration` propone el ensanchado como `ALTER COLUMN`; aplicarlo no pierde datos, pero en una tabla grande el motor puede reescribirla. |
| QK-22 | P2 | `internal/enginesuite/querybench_cases_test.go` (Q36, Q37) | `WithRecursive` emite `WITH RECURSIVE` sobre un cuerpo que no puede recurrir: toma una sola subconsulta y nada puede referenciar la CTE que se está definiendo desde dentro de su propio cuerpo. El nombre de la API afirma la capacidad; recorrer un árbol de categorías —la razón canónica para una CTE recursiva— no se puede. | **NO ERA UN DEFECTO** (quark#391). `UnionAll(...).AsSubquery()` compone ese cuerpo desde que entraron los operadores de conjunto. Lo que la medición leyó fue el comentario de `WithRecursive`, que seguía diciendo lo contrario: una nota que sobrevivió a lo que describía. Corregida, y el banco comprueba que un árbol de cuatro niveles vuelve entero. |
| QK-23 | P2 | `internal/enginesuite/querybench_cases_test.go` (Q33, Q44) | `With()` declara la CTE y nunca cambia el `FROM`, así que la tabla derivada queda declarada e ignorada salvo que se una explícitamente. Top-N por grupo emite un statement que corre y falla con `no such column: ranked.rn`. | **Medido de más** (quark#391). Unir la CTE ya funcionaba, y el top-N por grupo con ella también. Lo que faltaba era leer DE la CTE cuando las filas derivadas SON la consulta: `FromCTE`. |
| QK-25 | P2 | `internal/guard/guard.go:284`, `expr.go:126` | La whitelist de operadores del builder no incluye `ESCAPE`, y ninguna `Expr` puede emitirlo, así que un `LIKE` construido con Quark depende del escape POR DEFECTO del motor. PostgreSQL y MySQL toman la barra invertida y SQL Server las clases entre corchetes; **SQLite y Oracle no tienen ninguno**, de modo que un `%` o un `_` en el VALOR ensancha la coincidencia. Lo encontró el arnés de `quarkdatasource` al medir los operadores de filtro de A6: un `contains` de `%` devolvía las tres filas. El defecto es el mismo que arrastra la caja de búsqueda (`escapeLike` devuelve el texto intacto en esos dos motores), que sigue contestando de más. | Que el builder sepa emitir `LIKE … ESCAPE '\\'`, que los cinco motores aceptan, y que `escapeLike` escape también en SQLite y Oracle. Mientras tanto `quarkdatasource` **rechaza** la consulta en esos motores en vez de contestarla mal (orbit, A6/S5); la búsqueda no se tocó, porque cambiarla altera el comportamiento de una función publicada. |
| QK-26 | P1 | `tx.go:639-663` (`ForTx`) ; `client.go:516-580` (`For`) ; `rls_native.go:615-628` (`TenantRouter.Tx`) | **El confinamiento por tenant no sobrevive a una transacción.** `For[T]` aplica el aislamiento cuando el proveedor es un `TenantRouter` —`q.schema = tenantID` en `SchemaPerTenant`, `tenantID`/`tenantCol` más el `WHERE` inyectado en `RowLevelSecurityClient`—; `ForTx[T]` construye su `BaseQuery` **sin schema, sin tenantID, sin tenantCol y sin ese WHERE**, porque sólo recibe el `*Tx` y no tiene acceso al router. Y `TenantRouter.Tx` entrega un `*Tx` desnudo para toda estrategia que no sea `RowLevelSecurityNative`. Dentro de `router.Tx(...)`, una consulta lee filas de otros tenants y escribe en el schema por defecto. `DatabasePerTenant` se salva porque el aislamiento está en la conexión; el RLS nativo, porque lo impone el motor | Que el camino transaccional lleve el mismo aislamiento que el no transaccional: o `TenantRouter.Tx` construye un `*Tx` que recuerda su tenant, o `ForTx` lo toma del contexto. La sonda `RLS-13` del banco lo mide; el arreglo tiene que moverla a `present` |
| QK-27 | P2 | `migrate_execute.go:392-455` (`applyCreateTable`) ; godoc en `migrate_diff.go:41` y `migrate_execute.go:387` | **`ApplyPlan` aplica a medias una `OpCreateTable` y devuelve `nil`.** El op lleva la tabla entera —columnas, índices, claves ajenas y CHECK— y el ejecutor sólo lee las columnas: `t.Indexes`, `t.ForeignKeys` y `t.Checks` no aparecen en la función. Los dos godoc se contradicen y ninguno es cierto: uno dice que el ejecutor los emite, el otro que vienen en ops posteriores que `Diff` nunca produce para una tabla nueva. Es exactamente lo que `Plan.Down` se niega a hacer en la dirección inversa, donde prefiere `ErrIrreversibleOperation` antes que «un rollback que dice que fue bien y deja el esquema distinto» | Emitir la superficie no-columna, o rechazar el op con un error. Lo que no puede seguir es devolver `nil`. Los tests de motor no lo cazan porque ningún plan del CI lleva índices: `PlanMigration` los neutraliza antes |
| QK-28 | P2 | `internal/migrate/migrate.go:264-276` (`applyPrecisionScale`) | La pista `precision`/`scale` del tag `db` **descarta el tipo base en vez de refinarlo**, y no mira ni el tipo Go ni el dialecto: un `string` con esa pista sale `DECIMAL(10,2)` y un `bool` sale `DECIMAL(3)`. Su godoc describe lo contrario —dice que sólo se alcanza a través de un mapper— cuando el camino real es el switch embarcado, para cualquier campo con `precision>0` | Un guard por tipo Go, y emitir por dialecto. El linter de tags también la acepta en cualquier campo, así que el aviso puede llegar antes |
| QK-29 | P2 | `internal/migrate/migrate.go:75-81` (`SQLTypeWithOpts`) | **El camino documentado para UUID genera tablas sin clave primaria.** El mapper registrado retorna antes de que se añada el sufijo `PRIMARY KEY`, así que el ejemplo de `type_mapper.go` —copiado tal cual en la guía pública— produce una tabla cuya PK no existe | Añadir el sufijo después del mapper, o negarse a mapear el tipo de una PK. El control `TYP-02` del banco lo mide |
| QK-30 | P3 | `website/docs/reference/type-matrix.mdx` ; `website/docs/reference/roadmap.mdx:49` | La documentación publicada afirma capacidades que el banco mide como ausentes: la matriz dice que `[]string`, `[]int64` y `map[string]any` se serializan, y el roadmap, en la sección de lo que YA existe, que `Array[T]` es nativo en PostgreSQL | Corregirlas con lo medido. Nada compara la matriz publicada con lo que el generador emite: ése es el guard que falta |
| QK-31 | P3 | `internal/enginesuite/mariadb_suite_test.go:40,60,207,359` | Cuatro `t.Skipf("MariaDB not reachable")` DESPUÉS de comprobar que el motor no responde: **la lane se pone verde sin haber ejercido su motor**. Es la forma exacta que A6 dejó escrita con `ORBIT_BENCH_BROWSER=required` — una lane verde porque no había nada que medir | Que la lane exija su motor en CI y sólo tolere el salto en local, como hace orbit con el navegador |
| QK-24 | P3 | `internal/enginesuite/querybench_cases_test.go` (Q22, Q23) | `GroupBy` sin `Select()` deja la proyección en `SELECT *`, inválido con `GROUP BY` en todo motor salvo SQLite y MySQL en modo permisivo: pasa en el banco y fallaría en producción sobre PostgreSQL. | Movido a **A12**: desde quark#392 AVISA nombrando el arreglo, y convertirlo en error rompe a quien depende de los motores permisivos, así que su arreglo pertenece al major que QADR-0010 acumula. |
