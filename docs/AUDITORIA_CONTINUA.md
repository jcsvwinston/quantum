...
| umbrella-quickstart-embeds | paraguas | `bash scripts/check_quickstart_embeds.sh website/docs/quickstart.md` | Lo que el quickstart de la suite **embebe** (fences ```` ```go file=<rootDir>/… ```` que remark-code-import resuelve contra el submódulo nucleus pinado) existe al pin y dice lo que la prosa explica: cada fence se resuelve con las reglas del plugin (`<rootDir>`, `#Lx-Ly` 1-based inclusivo, sangría común fuera) y se contrasta con el `embeds:` del front matter (`'<ruta[#rango]> \| ^primera-línea \| subcadena… \| última-línea$'`, cruce en las dos direcciones) y con la regex de jerga de `check_served_jargon.sh` leída de su fuente. Caza el pin POR DETRÁS de la página (el ejemplo existe con el mismo nombre en el pin anterior, el build sale verde y `built-codeblocks` no lo ve porque el bloque no está vacío: se publicaría el `Module` sin `Policies`/`CSRFExempt` bajo el párrafo que los explica), el rango que se desplaza con una edición de la plantilla y el fichero que ya no existe (ENOENT en build). Sin transición: rojo con el pin viejo es información de certificación, la página del arco A2 sólo se fusiona dentro del PR del set que re-pina nucleus. Sin `embeds:`, sin página o sin submódulo es EXIT 2. |
...
dependencias; con `showcase-smoke` fuera de la lista, el lunes enrojecía sin
abrir issue).
